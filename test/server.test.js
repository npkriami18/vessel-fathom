...
test("poll drains queued comments", async () => {
  const fixture = await fixtureServer();
  try {
    await fixture.store.enqueue("http://localhost:3000", { text: "Fix checkout", sourceEventId: "event-1" });

    const first = await fetch(`${fixture.baseUrl}/api/poll?origin=${encodeURIComponent("http://localhost:3000/cart")}`);
    const second = await fetch(`${fixture.baseUrl}/api/poll?origin=${encodeURIComponent("http://localhost:3000")}`);
...
test("notify returns open timeline notifications", async () => {
  const fixture = await fixtureServer();
  try {
    await fixture.store.appendInteraction("http://localhost:3000", {
      id: "event-1",
...
test("notification approve queues a comment and dismiss removes it from notify", async () => {
  const fixture = await fixtureServer();
  try {
    await fixture.store.appendInteraction("http://localhost:3000", {
      id: "event-approve",
    ...
    const poll = await fetch(`${fixture.baseUrl}/api/poll?origin=${encodeURIComponent("http://localhost:3000")}`);

    assert.equal(approve.status, 200);
    assert.equal((await notify.json()).empty, true);
    assert.equal((await poll.json()).queue.length, 1);
  } finally {
    await fixture.close();
  }
});
...
test("chrome route and assets are served", async () => {
  ...
  try {
    ...
    assert.match(await js.text(), /async function refresh/);
  } finally {
    await fixture.close();
  }
});
...
test("proxy injects observer SDK into upstream HTML", async () => {
